...
def test_view_shows_active_marker(tmp_path: Path, monkeypatch):
    work = tmp_path / "claude-work"
    make_claude_json(work)
    config = tmp_path / "profiles.yaml"
    _write_registry(config, None, {"work": {"path": str(work)}})
...
def test_switch_action_does_not_change_marker(tmp_path: Path, monkeypatch):
    """The TUI cannot mutate the parent shell's env, so 's' shows a hint
    instead of changing the active marker. The marker follows CLAUDE_CONFIG_DIR."""
    work = tmp_path / "claude-work"
    make_claude_json(work)
    personal = tmp_path / "claude-personal"
    ...
    async def _run():
        ...
        async with app.run_test() as pilot:
            ...
            await pilot.press("s")
            await pilot.pause()
            # work is still active per the env; row order is alphabetical
            # (personal, work) so work is row 1
